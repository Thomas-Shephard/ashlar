...
{
    private static readonly StepUpRequirement TotpManagementRequirement = new(TimeSpan.FromMinutes(10));
    private static readonly StepUpRequirement RecoveryCodeManagementRequirement = new(TimeSpan.FromMinutes(10));
    ...
    private static async Task<IResult> StartTotpEnrollmentAsync([AsParameters] MfaEnrollServices services)
    {
        ...
        {
            var proof = services.HttpContext.CreateFreshMfaProof(services.StepUp, TotpManagementRequirement, ITotpService.ManagementProofPurpose);
        ...
        {
            var proof = services.HttpContext.CreateFreshPrimaryAuthenticationProof(services.StepUp, TotpManagementRequirement.FreshnessWindow, ITotpService.ManagementProofPurpose);
            if (!proof.Succeeded) return Results.Forbid();
    ...
    {
        var proof = services.HttpContext.CreateFreshMfaProof(services.StepUp, RecoveryCodeManagementRequirement, IRecoveryCodeService.ManagementProofPurpose);
        if (!proof.Succeeded)
        {
            return Results.Forbid();
        }
