...
internal static class AuthEndpoints
{
    public static void MapAuthEndpoints(this IEndpointRouteBuilder app)
    {
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            var callback = new Uri(new Uri(options.Value.PublicAppUrl), "/auth/magic-link/callback");
        ...
        app.MapGet("/auth/magic-link/callback", (string t) => AppViews.RenderMagicLinkCallback(t));

        app.MapPost("/auth/magic-link/callback", async Task<IResult> (
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            await signInManager.SignInAsync(httpContext, response.User.Id, cancellationToken: cancellationToken);
            return Results.Ok(new { userId = response.User.Id });
        });

        app.MapPost("/auth/logout", async (IAshlarSignInManager signInManager, HttpContext httpContext, CancellationToken cancellationToken) =>
        {
