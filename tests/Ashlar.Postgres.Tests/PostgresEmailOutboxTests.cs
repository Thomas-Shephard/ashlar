...
public sealed class PostgresEmailOutboxTests : PostgresTestBase
{
    ...
    [Test]
    public void SenderConstructorThrowsForNullConnectionProvider()
    {
        // ReSharper disable once NullableWarningSuppressionIsUsed
        Assert.Throws<ArgumentNullException>(() => _ = new PostgresEmailOutboxSender(null!));
    }
    ...
    public void DispatcherConstructorThrowsForNullArguments()
    {
        ...
        {
            Assert.Throws<ArgumentNullException>(() => _ = new PostgresEmailOutboxDispatcher<IEmailTransport>(null!, options));
            Assert.Throws<ArgumentNullException>(() => _ = new PostgresEmailOutboxDispatcher<IEmailTransport>(services, null!));
        }
    }

    [Test]
    public async Task SenderSendAsyncInsertsMessage()
    {
        var sender = new PostgresEmailOutboxSender(_serviceProvider.GetRequiredService<IPostgresConnectionProvider>());
        var message = new EmailMessage("to@example.com", "Subject", "Body", "<html></html>", new EmailMessageOptions
        {
        ...
        var row = await connection.QuerySingleAsync<RawOutboxRow>("""
            SELECT to_address AS ToAddress, from_address AS FromAddress, reply_to_address AS ReplyToAddress,
                   subject AS Subject, text_body AS TextBody, html_body AS HtmlBody,
                   headers AS Headers, metadata AS Metadata, attempt_count AS AttemptCount,
                   sent_at AS SentAt, failed_at AS FailedAt
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(row.ReplyToAddress, Is.EqualTo("replyto@example.com"));
            Assert.That(row.Subject, Is.EqualTo("Subject"));
            Assert.That(row.TextBody, Is.EqualTo("Body"));
            Assert.That(row.HtmlBody, Is.EqualTo("<html></html>"));
            Assert.That(row.Headers, Does.Contain("X-Test"));
            Assert.That(row.Metadata, Does.Contain("Test"));
    ...
    public async Task SenderSendAsyncIsTransactional()
    {
        ...
        var sender = new PostgresEmailOutboxSender(provider);
    ...
    public async Task DispatcherProcessBatchAsyncClaimsAndSendsMessages()
    {
        ...
        var services = new ServiceCollection();
        services.AddSingleton(transportMock.Object);
        ...
        var dispatcher = new PostgresEmailOutboxDispatcher<IEmailTransport>(
            dispatcherProvider,
        ...
        transportMock.Verify(t => t.DeliverAsync(It.Is<EmailMessage>(m => m.To == "to@example.com"), It.IsAny<CancellationToken>()), Times.Once);
        ...
        using (Assert.EnterMultipleScope())
        {
            Assert.That(row.SentAt, Is.Not.Null);
    ...
    {
        var transportMock = new Mock<IEmailTransport>();
        transportMock.Setup(t => t.DeliverAsync(It.IsAny<EmailMessage>(), It.IsAny<CancellationToken>()))
        ...
        var options = new PostgresEmailOutboxOptions { InitialRetryDelay = TimeSpan.FromSeconds(1) };
        var dispatcher = new PostgresEmailOutboxDispatcher<IEmailTransport>(
            dispatcherProvider,
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(row.LastAttemptAt, Is.Not.Null);
            Assert.That(row.AvailableAt, Is.GreaterThanOrEqualTo(row.LastAttemptAt.Value + options.InitialRetryDelay));
        }
    }
    ...
    public async Task DispatcherProcessBatchAsyncRespectsBatchSize()
    {
        ...
        transportMock.Verify(t => t.DeliverAsync(It.IsAny<EmailMessage>(), It.IsAny<CancellationToken>()), Times.Exactly(3));
    }
    ...
    public async Task CleanupIntegrationRemovesSentAndFailedEmails()
    {
        ...
        await connection.ExecuteAsync("""
            ...
            """, new { id1 = Guid.NewGuid(), id2 = Guid.NewGuid(), id3 = Guid.NewGuid(), id4 = Guid.NewGuid(), now, old });
    ...
    {
        var entry = new PostgresEmailOutboxDispatcher<IEmailTransport>.OutboxEntry
        {
            Id = Guid.NewGuid(),
        ...
        var message = PostgresEmailOutboxDispatcher<IEmailTransport>.MapToEmailMessage(entry);
    ...
    [Test]
    public async Task DispatcherExecuteAsyncStopsOnCancellation()
    {
        var transportMock = new Mock<IEmailTransport>();
        var services = new ServiceCollection();
        services.AddSingleton(transportMock.Object);
        ...
        Assert.That(dispatcher.ExecuteTask.IsCompleted, Is.True);
    }

    [Test]
    public void DispatcherExecuteAsyncThrowsForInvalidOptions()
    {
        var transportMock = new Mock<IEmailTransport>();
        var services = new ServiceCollection();
        services.AddSingleton(transportMock.Object);
        ...
        Assert.ThrowsAsync<InvalidOperationException>(() => dispatcher.ExecuteTask!);
    }

    [Test]
    public async Task DispatcherExecuteAsyncStopsWhenAlreadyCanceled()
    {
        var transportMock = new Mock<IEmailTransport>();
        var services = new ServiceCollection();
        services.AddSingleton(transportMock.Object);
        ...
        transportMock.Verify(t => t.DeliverAsync(It.IsAny<EmailMessage>(), It.IsAny<CancellationToken>()), Times.Never);
    }

    [Test]
    public async Task DispatcherExecuteAsyncContinuesImmediatelyWhenBatchIsFull()
    {
        var transportMock = new Mock<IEmailTransport>();
    ...
    [Test]
    public async Task DispatcherExecuteAsyncContinuesOnError()
    {
        var transportMock = new Mock<IEmailTransport>();
        var services = new ServiceCollection();
        services.AddSingleton(transportMock.Object);
    ...
    public async Task DispatcherProcessBatchAsyncHandlesLargeLastError()
    {
        var transportMock = new Mock<IEmailTransport>();
        transportMock.Setup(t => t.DeliverAsync(It.IsAny<EmailMessage>(), It.IsAny<CancellationToken>()))
            .ThrowsAsync(new InvalidOperationException(new string('X', 2000)));
    ...
    private async Task SeedMessageAsync(string to)
    {
        ...
        await connection.ExecuteAsync(
            ...
            new { id = Guid.NewGuid(), to, now = DateTimeOffset.UtcNow });
    }
