...
internal sealed class SqliteSecurityEventWebhookOutboxDiagnosticsTests : SqliteTestBase
{
    ...
    public async Task CheckAsyncCountsOutboxBucketsAndOldestTimestamps()
    {
        ...
        await ExecuteAsync("""
            ...
            VALUES ($id1, 'audit', 'https://example.test/old', $eventId1, 'security.test', $oldPending, 1000, $body, $headers, $oldPending, $oldPending),
                   ($id2, 'audit', 'https://example.test/new', $eventId2, 'security.test', $newPending, 1000, $body, $headers, $newPending, $newPending),
                   ($id3, 'audit', 'https://example.test/scheduled', $eventId3, 'security.test', $scheduled, 1000, $body, $headers, $scheduled, $scheduled);
            ...
            VALUES ($id4, 'audit', 'https://example.test/locked', $eventId4, 'security.test', $oldPending, 1000, $body, $headers, $oldPending, $oldPending, $lockedUntil, 'worker'),
                   ($id5, 'audit', 'https://example.test/expired', $eventId5, 'security.test', $oldPending, 1000, $body, $headers, $oldPending, $oldPending, $expiredLock, 'worker');
            ...
            VALUES ($id6, 'audit', 'https://example.test/failed-old', $eventId6, 'security.test', $oldPending, 1000, $body, $headers, $oldPending, $oldPending, $oldestFailed),
                   ($id7, 'audit', 'https://example.test/failed-new', $eventId7, 'security.test', $oldPending, 1000, $body, $headers, $oldPending, $oldPending, $newestFailed);
            """, command =>
        {
