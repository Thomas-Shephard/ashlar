...
internal sealed class SecurityEventWebhookOutboxDiagnosticsRunnerTests
{
    ...
    {
        var runner = new SecurityEventWebhookOutboxDiagnosticsRunner("TestProvider");
        ...
        var result = await runner.CheckAsync(
            ...
            new SecurityEventWebhookOutboxDiagnosticsContext<TestConnection>(
                _ => new ValueTask<TestConnection>(new TestConnection()),
                (_, _) => Task.FromResult(true),
                (_, _, _) => Task.FromResult(new SecurityEventWebhookOutboxDiagnosticSnapshot
                {
                    PendingCount = 1,
                    ScheduledCount = 2,
                    LockedCount = 3,
            ...
            new SecurityEventWebhookOutboxDiagnosticOptions(6, TimeSpan.FromSeconds(7), 8));
    ...
    public void CheckAsyncRejectsNullArguments()
    {
        ...
        Func<TestConnection, DateTimeOffset, CancellationToken, Task<SecurityEventWebhookOutboxDiagnosticSnapshot>> querySnapshot =
            (_, _, _) => Task.FromResult(new SecurityEventWebhookOutboxDiagnosticSnapshot());
        Action<Exception> logException = _ => { };
        var context = new SecurityEventWebhookOutboxDiagnosticsContext<TestConnection>(openConnection, tableExists, querySnapshot, logException);
        var options = new SecurityEventWebhookOutboxDiagnosticOptions(1, TimeSpan.FromSeconds(1), 1);
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.ThrowsAsync<ArgumentNullException>(async () => await runner.CheckAsync(timeProvider, new SecurityEventWebhookOutboxDiagnosticsContext<TestConnection>(null!, tableExists, querySnapshot, logException), options));
            Assert.ThrowsAsync<ArgumentNullException>(async () => await runner.CheckAsync(timeProvider, new SecurityEventWebhookOutboxDiagnosticsContext<TestConnection>(openConnection, null!, querySnapshot, logException), options));
            Assert.ThrowsAsync<ArgumentNullException>(async () => await runner.CheckAsync(timeProvider, new SecurityEventWebhookOutboxDiagnosticsContext<TestConnection>(openConnection, tableExists, null!, logException), options));
            Assert.ThrowsAsync<ArgumentNullException>(async () => await runner.CheckAsync(timeProvider, new SecurityEventWebhookOutboxDiagnosticsContext<TestConnection>(openConnection, tableExists, querySnapshot, null!), options));
        }
    }
