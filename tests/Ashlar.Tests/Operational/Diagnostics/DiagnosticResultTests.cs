...
internal sealed class DiagnosticResultTests
{
    ...
    [Test]
    public void SecurityEventWebhookOutboxDiagnosticResultCanBeConstructedWithExpectedValues()
    {
        var checkedAt = new DateTimeOffset(2026, 5, 20, 12, 0, 0, TimeSpan.Zero);
        var oldestPendingAt = checkedAt.AddMinutes(-10);
        var oldestFailedAt = checkedAt.AddHours(-1);
    ...
    [Test]
    public void SecurityEventWebhookOutboxDiagnosticSnapshotCanBeConstructedWithExpectedValues()
    {
        var oldestPendingAt = new DateTimeOffset(2026, 5, 20, 11, 50, 0, TimeSpan.Zero);
        var oldestFailedAt = new DateTimeOffset(2026, 5, 20, 11, 0, 0, TimeSpan.Zero);

        var snapshot = new SecurityEventWebhookOutboxDiagnosticSnapshot
        {
            PendingCount = 10,
            ScheduledCount = 3,
            LockedCount = 2,
    ...
    [Test]
    public void SecurityEventWebhookOutboxDiagnosticOptionsCanBeConstructedWithExpectedValues()
    {
        var options = new SecurityEventWebhookOutboxDiagnosticOptions(8, TimeSpan.FromSeconds(15), 25);
