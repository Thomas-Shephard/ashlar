...
internal sealed class CredentialServiceTests
{
    ...
    public async Task LinkCredentialAsyncShouldUseClockForCreatedAt()
    {
        ...
        await _service.LinkCredentialAsync(LinkRequest(userId, assertionMock.Object, providerMock.Object));
    ...
    public async Task ResolveAsyncShouldRespectUserIdFromContextWhenProviderFindUserReturnsNull()
    {
        ...
        providerMock.As<IAuthenticationUserResolver>().Setup(p => p.FindUserAsync(assertionMock.Object, context, It.Is<IUserLookup>(lookup => !typeof(IUserRepository).IsInstanceOfType(lookup)), It.IsAny<CancellationToken>()))
            .Returns((IAuthenticationAssertion _, AuthenticationContext _, IUserLookup lookup, CancellationToken cancellationToken) =>
                lookup.GetUserByIdAsync(Guid.NewGuid(), cancellationToken));
    ...
    [Test]
    public async Task ResolveAsyncShouldUseContextUserIdWhenProviderHasNoUserResolver()
    {
        var userId = Guid.NewGuid();
        var user = new User { Id = userId, DisplayEmail = "test@example.com" };
        var assertionMock = new Mock<IAuthenticationAssertion>();
        ...
        var (resolvedUser, resolvedCredential, _, _) = await _service.ResolveAsync(new AuthenticationContext(UserId: userId), assertionMock.Object, providerMock.Object);

        using (Assert.EnterMultipleScope())
        {
            Assert.That(resolvedUser, Is.EqualTo(user));
            Assert.That(resolvedCredential, Is.Not.Null);
            Assert.That(providerMock.Object, Is.Not.InstanceOf<IAuthenticationUserResolver>());
        }
    }

    [Test]
    public async Task ResolveAsyncShouldUseCredentialResolverWithoutEmptyProviderKeySentinel()
    {
        var userId = Guid.NewGuid();
    ...
    [Test]
    public async Task ResolveAsyncShouldUseDummyLookupWhenCredentialResolverReturnsNull()
    {
        var userId = Guid.NewGuid();
        var assertionMock = new Mock<IAuthenticationAssertion>();
        var providerMock = new Mock<IAuthenticationProvider>();
        ...
        _credentialRepositoryMock.Verify(r => r.GetCredentialForUserAsync(userId, ProviderType.RecoveryCode, "RecoveryCode", It.Is<string>(key => key.Length == 32), It.IsAny<CancellationToken>()), Times.Once);
    }

    [Test]
    public async Task ResolveAsyncShouldTreatEmptyDefaultProviderKeyAsDummyLookup()
    {
        var userId = Guid.NewGuid();
        var assertionMock = new Mock<IAuthenticationAssertion>();
        var providerMock = new Mock<IAuthenticationProvider>();
        ...
        Assert.That(resolvedCredential, Is.Null);
        _credentialRepositoryMock.Verify(r => r.GetCredentialForUserAsync(userId, ProviderType.Oidc, "Google", It.Is<string>(key => key.Length == 32), It.IsAny<CancellationToken>()), Times.Once);
        _credentialRepositoryMock.Verify(r => r.GetCredentialForUserAsync(userId, ProviderType.Oidc, "Google", "", It.IsAny<CancellationToken>()), Times.Never);
    }
    ...
    public async Task LinkCredentialAsyncShouldCreateCredential()
    {
        ...
        await _service.LinkCredentialAsync(LinkRequest(userId, assertionMock.Object, providerMock.Object, "raw", credentialMetadata));
    ...
    {
        Assert.ThrowsAsync<ArgumentException>(() => _service.LinkCredentialAsync(LinkRequest(Guid.Empty, new Mock<IAuthenticationAssertion>().Object, new Mock<IAuthenticationProvider>().Object)));
    }
    ...
    public async Task LinkCredentialAsyncWithMissingUserShouldFail()
    {
        ...
        var result = await _service.LinkCredentialAsync(LinkRequest(userId, new Mock<IAuthenticationAssertion>().Object, new Mock<IAuthenticationProvider>().Object));
    ...
    public async Task LinkCredentialAsyncWithEmptyProviderKeyShouldFail()
    {
        ...
        var result = await _service.LinkCredentialAsync(LinkRequest(userId, assertionMock.Object, providerMock.Object));
    ...
    {
        // ReSharper disable once NullableWarningSuppressionIsUsed
        Assert.ThrowsAsync<ArgumentNullException>(() => _service.LinkCredentialAsync(LinkRequest(Guid.NewGuid(), null!, new Mock<IAuthenticationProvider>().Object)));
    }
    ...
    {
        // ReSharper disable once NullableWarningSuppressionIsUsed
        Assert.ThrowsAsync<ArgumentNullException>(() => _service.LinkCredentialAsync(LinkRequest(Guid.NewGuid(), new Mock<IAuthenticationAssertion>().Object, null!)));
    }
