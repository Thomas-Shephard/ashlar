...
Fresh MFA and step-up support is built from session authentication metadata plus the framework-neutral `IStepUpAuthenticationService`. The evaluator answers whether a session is active and has recent additional verification within a required freshness window, optionally constrained to allowed providers or factor names. Hosts cannot mark a session fresh from caller-supplied provider, factor, or session metadata. Ashlar marks step-up freshness only after it verifies an allowed factor and returns the corresponding `MfaAuthenticationResult` or `AuthenticationResponse` capability carrying its internal step-up marking proof. Passing that capability to `MarkVerifiedAsync` updates only the proven active session and stores no raw codes, recovery codes, passkey ceremony JSON, tokens, password hashes, or protected payloads.
...
Rate-limit bucket search, detail, and reset are global operational administration. Their PostgreSQL, SQLite, and Redis stores do not record tenant ownership, so administration requires explicit `OperationalAdministrationScope.Global` scope and host authorization across all tenant origins.
...
- **Single Connection, Sequential Work**: One durable root transaction is supported per scope. Nested calls in the same async flow join that root and must complete in stack order. Concurrent inherited transaction work, including `Task.WhenAll` branches sharing the provider connection, is rejected with `InvalidOperationException`; await that work sequentially instead.
- **Exact Composition**: Durable repositories, audit sinks, and outbox handlers are verified by instance identity so every mutation uses the provider's transaction. DI decorators must be composed by the persistence provider inside that boundary; decorating an enlisted service afterward replaces the verified instance and durable service resolution will reject it.
