...
Direct callers that use Ashlar's generic factor pipeline with `PasskeyAssertion` must pass an assertion with `UserVerified = true`; unverified user-verification assertions are rejected before credential resolution. `IPasskeyService.StartFactorAsync` and `CompleteFactorAsync` handle this policy for normal passkey factor flows.
