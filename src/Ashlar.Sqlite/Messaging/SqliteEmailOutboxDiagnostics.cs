        ...
        CancellationToken cancellationToken)
    {
        ...
        command.CommandText = """
            SELECT
                ...
                COALESCE(SUM(CASE WHEN sensitivity IS NOT 'Normal' AND sent_at IS NULL AND failed_at IS NULL AND discarded_at IS NULL AND available_at <= $now AND (locked_until IS NULL OR locked_until <= $now) THEN 1 ELSE 0 END), 0) AS sensitive_pending_count,
                COALESCE(SUM(CASE WHEN sensitivity IS NOT 'Normal' AND sent_at IS NULL AND failed_at IS NULL AND discarded_at IS NULL AND available_at > $now THEN 1 ELSE 0 END), 0) AS sensitive_scheduled_count,
                COALESCE(SUM(CASE WHEN sensitivity IS NOT 'Normal' AND sent_at IS NULL AND failed_at IS NULL AND discarded_at IS NULL AND locked_until IS NOT NULL AND locked_until > $now THEN 1 ELSE 0 END), 0) AS sensitive_locked_count,
                COALESCE(SUM(CASE WHEN sensitivity IS NOT 'Normal' AND failed_at IS NOT NULL AND discarded_at IS NULL THEN 1 ELSE 0 END), 0) AS sensitive_failed_count,
