        ...
        CancellationToken cancellationToken)
    {
        ...
        command.CommandText = """
            SELECT
                ...
                COALESCE(SUM(CASE WHEN sent_at IS NULL AND failed_at IS NULL AND locked_until IS NOT NULL AND locked_until > $now THEN 1 ELSE 0 END), 0) AS locked_count,
                ...
                COALESCE(SUM(CASE WHEN sensitivity = 'ContainsLiveSecret' AND sent_at IS NULL AND failed_at IS NULL AND locked_until IS NOT NULL AND locked_until > $now THEN 1 ELSE 0 END), 0) AS sensitive_locked_count,
