...
public static class AshlarCleanupPlan
{
    ...
    [
        ...
        new("discarded_security_event_webhooks", SecurityEventWebhookOutboxTable, $"discarded_at IS NOT NULL AND discarded_at < {CutoffToken}", "discarded_at", options => options.RemoveDiscardedSecurityEventWebhooksAfter, result => result.DiscardedSecurityEventWebhooks, (result, count) => result with { DiscardedSecurityEventWebhooks = result.DiscardedSecurityEventWebhooks + count }),
